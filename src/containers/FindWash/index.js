...
import { Text, View, StyleSheet, TextInput, Image, TouchableOpacity, BackHandler, ScrollView, TouchableHighlight, Picker , Alert} from "react-native";
  ...
  return (
    <GooglePlacesAutocomplete
      ...
      onPress={(data, details = null) => { // 'details' is provided when fetchDetails = true
        ...
        getlocation.getpicklatLng(details.geometry.location.lat, details.geometry.location.lng);
      }}
...
class FindWashScreen extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      detergentErr:false,
      bagErr:false,
      pickup: { latitude: null, longitude: null, latitudeDelta: 0.015, longitudeDelta: 0.0121 },
      currentLocation: null,
      showDropoff: false,
      showPickup: true,
      showForm: false,
      isCheckedExperience: false,
      dropoff: { latitude: null, longitude: null, latitudeDelta: 0.015, longitudeDelta: 0.0121 },
    };
  }
  componentDidMount() {
    ...
    Geolocation.getCurrentPosition(info => {
      this.setState({ pickup: { latitude: info.coords.latitude, longitude: info.coords.longitude, latitudeDelta: 0.015, longitudeDelta: 0.0121 }, dropoff: { latitude: info.coords.latitude, longitude: info.coords.longitude, latitudeDelta: 0.015, longitudeDelta: 0.0121 } })

      console.log("asasassssssssssssssssssssssssssss", info)
    });
    this.findCords();
  ...
  dropoffbackbutton = () => {
    this.setState({
      ...
      showPickup: true, showDropoff: false
    });
  ...
  formbackbutton = () => {
    this.setState({
      // dropoff: { lat: '', lng: '' }, 
      showDropoff: true, showForm: false
    });
  };
  ...
  renderDropDown = (headerText, placeholder, ErrTxt, Iserr, onChangeText, image, ) => {
  ...
  renderInputfield = (headerText, placeholder, ErrTxt, Iserr, onChangeText, image, enable) => {
  ...
  renderRadio = (headerText, placeholder1, placeholder2, ErrTxt, Iserr, tooltip, onChangeRadio) => {
    return (
      <View style={styles.inputFieldView}>
        ...
        {!tooltip && <Text style={styles.inputFieldHeaderText}>{headerText}</Text>}
        <View
          style={{
            flexDirection: "row",
            alignItems: "center",
  ...
  Handlepickup = () => {
    this.setState({
      ...
      showPickup: false, showDropoff: true
    });
  }
  handleDropOff = () => {
    this.setState({
      // dropoff: { lat: '24.8681357', lng: '67.0442549' }, 
      showDropoff: false, showForm: true, toolTipVisible: true
    });
  ...
  getpicklatLng = (lat, lng) => {
    this.setState({ pickup: { latitude: lat, longitude: lng, latitudeDelta: 0.015, longitudeDelta: 0.0121 } })
  }
  getdroplatLng = (lat, lng) => {
    this.setState({ dropoff: { latitude: lat, longitude: lng, latitudeDelta: 0.015, longitudeDelta: 0.0121 } })
  }
    ...
    return (
      ...
      >
        ...
        <View style={{
          ...
          {this.state.pickup.latitude !== null && this.state.pickup.longitude !== null && <MapView
          ...
          >
            <Marker
              ...
              onDragEnd={(e) => {
                this.setState({ pickup: { latitude: e.nativeEvent.coordinate.latitude, longitude: e.nativeEvent.coordinate.longitude, latitudeDelta: 0.015, longitudeDelta: 0.0121 } })
              }}
            />
          </MapView>}
      ...
      {this.state.pickup && <TouchableOpacity
          onPress={() => {
            this.Handlepickup();
          }}
          style={[styles.submitButtonView, {marginLeft: Metrics.screenWidth * 0.05}]}
        >
          {/* <Image
  ...
  renderDropOff = () => {
    return (
      ...
      {this.state.dropoff && <TouchableOpacity
          onPress={() => {
            this.handleDropOff();
          }}
          style={[styles.submitButtonView, {marginLeft: Metrics.screenWidth * 0.05}]}
